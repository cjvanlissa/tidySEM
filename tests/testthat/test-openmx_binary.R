...
#   saveRDS(df, "test-openmx_binary_df.rds")
  df <- readRDS(testthat::test_path("test-openmx_binary_df.rds"))
  #df <- data_mix_ordinal
...
u4 | t4{C}*t1", classes = 2, data = df, run = FALSE)
  ...
  mix_tidysem <- OpenMx::mxModel(model = "mix",
                                 ...
                                 OpenMx::mxMatrix(values=c(1, 0), nrow=1, ncol=2, lbound = 1e-4, free=c(FALSE, TRUE), name="weights"),
                                 OpenMx::mxExpectationMixture(paste0("class", 1:2), weights = "weights", scale="sum"),
                                 OpenMx::mxFitFunctionML()
  )

  #dput(res_tidysem$class2$mat_dev$values, file = "clipboard")

  res_tidysem <- OpenMx::mxTryHardOrdinal(mix_tidysem)
