...
test_that("table_results works with mx_mixture multi-class output", {
  # Create simple data for mixture model
  set.seed(123)
  df <- data.frame(
    x = c(rnorm(50, mean = 0, sd = 1), rnorm(50, mean = 3, sd = 1))
  ...
  # Run mx_mixture with multiple classes - returns mixture_list
  mix <- mx_mixture(
    model = "x ~ m{C}*1
             x ~~ v{C}*x",
    classes = 1:2,
  ...
  # Multiple classes returns mixture_list
  expect_true(inherits(mix, "mixture_list"))
  expect_equal(length(mix), 2)
  ...
  # Should have results from both class solutions
  expect_true(length(unique(result$model)) >= 1)
})

test_that("table_results works with mx_mixture multi-class and labels", {
  set.seed(123)
  df <- data.frame(
    x = c(rnorm(50, mean = 0, sd = 1), rnorm(50, mean = 3, sd = 1))
  )
  ...
  result <- table_results(mix, use_labels = TRUE, dictionary = dict)
  expect_s3_class(result, "data.frame")
  expect_true(nrow(result) > 0)
  expect_true(label_contains(result, "Test Variable"))
})

test_that("table_results.mixture_list use_labels = FALSE ignores dictionary", {
  set.seed(123)
  df <- data.frame(
    x = c(rnorm(50, mean = 0, sd = 1), rnorm(50, mean = 3, sd = 1))
  )
  ...
  dict <- data.frame(
    name = "x",
    label = "Labeled Variable",
    stringsAsFactors = FALSE
  )
  
  result <- table_results(mix, use_labels = FALSE, dictionary = dict)
  
  expect_s3_class(result, "data.frame")
  
  # Labels should NOT be present when use_labels = FALSE
  expect_false(label_contains(result, "Labeled Variable"))
})