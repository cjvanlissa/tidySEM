...
model <- mx_mixture.data.frame(df, 2, run = FALSE, run_mixture_starts = FALSE)
# Estimate the model
res <- mx_mixed_lca(data = df, classes = 2)
#' }
#' }

mx_mixture.data.frame <- function(data = NULL,
                                  classes = 1L,
                                  variances = "equal",
                                  covariances = "zero",
                                  run = TRUE,
  ...
  if(length(variances) == 1){
    cl[["model"]] <- tidySEM:::profile_syntax(variances, covariances, names_cont)
    mix_profiles <- eval.parent(cl)
  } else {
    out <- mapply(function(v, c){
      cl[["model"]] <- profile_syntax(variances = v, covariances = c, names_cont)
      eval.parent(cl)
  ...
  if(classes == 1){
    ...
    thresh <- mx_threshold(vars = names(data)[vars_ord], nThresh = (sapply(df_ord, function(x){length(levels(x))})-1L), free = TRUE, values = mx_data_quantiles(df_ord))
