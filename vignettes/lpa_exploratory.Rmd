...
p <- ggplot(df_plot, aes(x = Value)) +
  ...
  facet_wrap(~Variable)+
  theme_bw()
ggsave("appendices/plot_gmm_desc.pdf", p, device = "pdf", width = 100, height = 100, units = "mm")
```
...
df_plot <- reshape(df_plot, varying = names(df_plot), direction = "long",
                   timevar = "Variable")
knitr::include_graphics("appendices/plot_gmm_desc.pdf")
```
...
p <- ggplot(df_plot, aes(x = Value)) +
  ...
  facet_wrap(~Variable)+
  theme_bw()
ggsave("appendices/plot_gmm_desc_log.pdf", p, device = "pdf", width = 100, height = 100, units = "mm")
```
```{r figdesclog, echo = FALSE, eval = pkgdown}
knitr::include_graphics("appendices/plot_gmm_desc_log.pdf")
```
...
p <- ggplot(df, aes(x = length, y = width)) +
  geom_point(alpha = .1) +
  theme_bw()
ggsave("appendices/plot_gmm_scatter.pdf", p, device = "pdf", width = 100, height = 100, units = "mm")
```
```{r figscatter, eval = pkgdown, echo = FALSE}
knitr::include_graphics("appendices/plot_gmm_scatter.pdf")
```
...
We will set the maximum number of classes $K$ to four to limit computational demands.
...
res <- mx_profiles(data = df,
                   ...
                   covariances = c("zero", "equal",
                                   "varying"),
                   expand_grid = TRUE)
saveRDS(res, "appendices/res_gmm.RData")
fit <- table_fit(res)
fit$par_ratio <- (5605*fit$n_min) / (fit$Parameters/fit$Classes)
saveRDS(fit, "appendices/gmm_tabfit.RData")
...
fit <- readRDS("appendices/gmm_tabfit.RData")
fit$np_ratio <- fit$n / fit$Parameters
fit$np_local <- (fit$n*fit$n_min) / ((fit$Parameters - (fit$Classes-1))/fit$Classes)
fit[c("lmr_lr", "lmr_p")] <- NULL
```
...
The global ratio of observations per parametrer is large, as the minimum `np_ratio` is 244.
...
names(tbl) <- c("Name", "LL", "p", "BIC", "Ent.",
         ...
         #"lmr_p", 
         "np_ratio", "np_local")
papaja::apa_table(tbl, caption = "Model fit table. Some columns removed to fit page.")
```
...
ggsave("appendices/gmm_plotfit.pdf", p, device = "pdf", width = 100, height = 100, units = "mm", scale = 1)
```
```{r echo = FALSE, eval = pkgdown, fig.cap="Bivariate profile plot", out.width="100%"}
knitr::include_graphics("appendices/gmm_plotfit.pdf")
```
...
saveRDS(results, "appendices/gmm_results.RData")
```
```{r eval = FALSE, echo = FALSE}
results <- readRDS("appendices/gmm_results.RData")
papaja::apa_table(results, digits = 2, caption = "Results of a 2-class model with free (co)variances")
```
...
ggsave("appendices/gmm_bivariate_bic.pdf", p, device = "pdf", width = 210, height = 100, units = "mm", scale = 1.5)
```
```{r echo = FALSE, eval = pkgdown, fig.cap="Bivariate profile plot", out.width="100%"}
knitr::include_graphics("appendices/gmm_bivariate_bic.pdf")
```
...
aux_pt <- BCH(res_final, model = "poly_typeOther | t1
                                  ...
                                  poly_typePP | t1", data = df_pt)
aux_pt <- mxTryHardOrdinal(aux_pt)
saveRDS(aux_pt, "appendices/gmm_aux_pt.RData")
```
```{r echo = FALSE, eval=FALSE}
aux_pt <- readRDS("appendices/gmm_aux_pt.RData")
```
