...
We can do the same for the nodes, but at this time, the mean structure is **not** part of the model, so `table_results()` will not retrieve any parameter estimates for the nodes.
Unless `label = NULL`, `get_nodes()` will use the node name as a label when no further information is available.
...
it is also possible to request auxiliary columns, and use these to customize the labels (or other properties of the plot) later.
By default, all available columns are included
Below is an example for `get_edges()`, but the same mechanism applies to `get_nodes()`:
...
Let's estimate a model and look at all of the coluns extracted by `get_edges()`:
...
They make use of the generic function `all_fun()`, and are documented in its help file (see `?all_fun`).

The function `all_fun()` takes an expression and a condition as arguments.
...
```{r}
prepare_graph(fit) %>%
  edit_graph({ color = "black" }, element = c("nodes", "edges")) %>% # Add color column to the graph elements
  all_fun({color = "blue"}, grepl("4", est), element = c("nodes", "edges")) %>% # Conditionally change color to blue when the column 'est' contains the number 4
  plot()
```
