...
#> Wald tests for linear hypotheses:
#>                 Hypothesis df  chisq    p
#>  model.Thresholds[2,3] = 1  1 0.0089 0.92
```
...
#> Wald tests for linear hypotheses:
#>              Hypothesis df chisq    p
#>  model.mat_dev[1,1] = 0  1  0.65 0.42
```
...
#> Wald tests for linear hypotheses:
#>                                 Hypothesis df  chisq    p
#>  model.mat_dev[1,3]+model.mat_dev[2,3] = 1  1 0.0089 0.92
```
...
While strictly positive deviances may aid in achieving model convergence, once convergence has been reached,
it is possible to reparametrize the model to use thresholds instead.
At present, this is supported by the utility function `deviances_to_thresholds()`, which takes in a model with deviances and returns a model with thresholds.
The model is automatically re-run.
