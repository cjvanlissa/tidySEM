...
2. Convert the data to a `tidy_sem` obejct by running `model <- tidy_sem(data)`
...
7. Run the analysis, either by:
    * Converting the `tidy_sem` object to `lavaan` syntax using `as.lavaan(model)` and using that as input for the `lavaan` functions `sem`, `lavaan`, or `cfa`
    * Converting the `tidy_sem` object to `Mplus` using `as.mplus(model)`, and using that as input for `MplusAutomation::mplusObject()`
...
```{r}
res <- lavaan(as.lavaan(model), data = df)
