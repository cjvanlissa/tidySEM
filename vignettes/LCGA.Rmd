---
title: "Tutorial: Latent Class Growth Analysis"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Tutorial: Latent Class Growth Analysis}
...
```{r eval = !pkgdown, echo = FALSE, results = "asis"}
cat("This vignette has been stripped down to comply with CRAN package size policies. To view the complete vignette, including graphics, see the [package website](https://cjvanlissa.github.io/tidySEM).")
```
...
```
```{r, echo = FALSE, out.width="50%", eval = pkgdown}
...
```
```{r, echo = FALSE, eval = pkgdown, out.width="50%"}
...
```
```{r tabfit, eval = TRUE, echo = FALSE, results = "asis"}
# Get fit table fit
...
```
```{r tabres, echo=FALSE, eval = TRUE, results='asis'}
...
```
```{r waldtests, echo = FALSE, eval = TRUE, results='asis'}
