...
saveRDS(res_1_3, "res_1_3.RData")
```
```{r eval = TRUE, echo = FALSE}
res_1_3 <- readRDS("res_1_3.RData")
```
...
set.seed(1)
res_blrt <- BLRT(res_1_3, replications = 100)
saveRDS(res_blrt, "res_blrt_mixed_lca.RData")
```

```{r eval = TRUE, echo = FALSE}
res_blrt <- readRDS("res_blrt_mixed_lca.RData")
res_blrt
```
...
saveRDS(res_pmc_srmr, "res_pmc_srmr.RData")
```
...
```
```{r echo = FALSE}
if(file.exists("res_pmc_srmr.RData")){
  res_pmc_srmr <- readRDS("res_pmc_srmr.RData")
  print(res_pmc_srmr)
}
```
