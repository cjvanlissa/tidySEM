...
if(!file.exists("lpa.yml")){
  out <- list()
} else {
  out <- yaml::read_yaml("lpa.yml")
}
pkgdown <- FALSE
```
...
p <- ggplot(df_plot, aes(x = Value)) +
  ...
  facet_wrap(~Variable)+
  theme_bw()
ggsave("plot_lpa_desc.svg", p, device = "svg", width = 100, height = 100, units = "mm")
```
```{r figdesc, echo = FALSE, eval = pkgdown}
knitr::include_graphics("plot_lpa_desc.svg")
```
